...
class CsvUpdater(Thread):
    ...
    def __init__(self, player_ref_queue, filepath, no_more_to_update: Event):
    ...
    def _logger(self):
        csv_content = pd.read_csv(self._filepath, dtype=self._dtypes)
        while True:
            if self._stop_event.isSet():
                break
            if not self.player_ref_queue.empty():
                player_ref = self.player_ref_queue.get()
                player_in_csv = csv_content[GeneralPlayerData.FutwizLink].eq(player_ref.href).any()
    ...
    def _write_new_player_to_csv(self, player_ref):
        ...
        with open(self._filepath, 'a', newline='', encoding="utf-8") as csvfile:
            ...
            print(f"New Player Added: {player_data[GeneralPlayerData.Name]}, {player_data[GeneralPlayerData.Version]}")
